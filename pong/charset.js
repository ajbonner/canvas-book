...
function drawChar(ctx, character, pos, size) {
  ...
  for (i = 0; i < glyphHeight; i++) {
    ...
    for (j = 0; j < glyphWidth; j++) {
      // use 1 bit mask as we only care about the first bit
      if (scanLine & 1) {
        ctx.fillStyle = '#ffffff';
        ctx.beginPath();
        // rendering is occuring right -> left
        ctx.rect(pos.x + (glyphWidth * size) - (j * size) - glyphWidth / 2,
          pos.y + (i * size), size, size);
        ctx.fill();
      }
